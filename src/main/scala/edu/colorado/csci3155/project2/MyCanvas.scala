...
import java.awt.geom.{Ellipse2D, Rectangle2D} //this is not used.
...
sealed trait Figure {
    ...
    def rotate(angRad: Double):Figure//added allow definitions in polygon and MyCanvas
...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) = (cList.minBy(_._1)._1, cList.maxBy(_._1)._1,cList.minBy(_._2)._2,cList.maxBy(_._2)._2)
    //TODO: Create a new polygon by shifting each vertex in cList by (x,y)
    //    Do not change the order in which the vertices appear
    override def translate(shiftX: Double, shiftY: Double): Polygon = Polygon(cList.map(c => (c._1 + shiftX, c._2 + shiftY)))
   //add rotation function so we can use map in class MyCanvas
    override def rotate(angRad: Double): Polygon = Polygon(cList.map(v => (v._1 * math.cos(angRad) - v._2 * math.sin(angRad), v._1 * math.sin(angRad) + v._2 * math.cos(angRad))))
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double) = {
        //this can be simplified further. Should do that.
        val allCoor: List[(Double,Double)] = List( (c._1 + r, c._2),(c._1 , c._2 + r),(c._1 - r, c._2),(c._1, c._2 - r) )
        (allCoor.minBy(_._1)._1, allCoor.maxBy(_._1)._1,allCoor.minBy(_._2)._2,allCoor.maxBy(_._2)._2)
    }

    //TODO: Create a new circle by shifting the center
    //simply shift the x and y coor and redraw object. radius is the same
    override def translate(shiftX: Double, shiftY: Double): MyCircle = MyCircle((c._1 + shiftX, c._2 + shiftY), r)
    override def rotate(angRad: Double): MyCircle = MyCircle( (c._1*math.cos(angRad) - c._2*math.sin(angRad), c._1*math.sin(angRad) + c._2*math.cos(angRad)),r)
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
       val x =  listOfObjects.map(x => x.getBoundingBox)
        (x.minBy(_._1)._1, x.maxBy(_._2)._2,x.minBy(_._3)._3,x.maxBy(_._4)._4)
    }

    //TODO: Write a function to translate each figure in the canvas by shiftX, shiftY
    //use each objects bulit in translate function
    def translate(shiftX: Double, shiftY: Double): MyCanvas = new MyCanvas( listOfObjects.map(x => x.translate(shiftX,shiftY)) )
    ...
    def placeRight(myc2: MyCanvas):MyCanvas = {
        // [xmin,xmax,xmin,ymax]
        val bb1 = getBoundingBox // references to the first call of MyCanvas
        val bb2 = myc2.getBoundingBox // this canvas myc2 is created with call to function place right
        //translate the second canvas using the translate function
        val c_hat = myc2.translate(bb1._2 - bb1._1, (bb1._4 - bb1._3)/2 - (bb2._4 - bb2._3)/2 )
        overlap(c_hat)//no need for obj.overlap as we are in the class for this obj.
    }
    ...
    def placeTop(myc2: MyCanvas): MyCanvas ={
        // [xmin,xmax,xmin,ymax]
        val bb1 = getBoundingBox
        val bb2 = myc2.getBoundingBox
        //translate the second canvas using the translate function
        val c_2 = myc2.translate( (bb1._2 - bb1._1)/2 - (bb2._2 - bb2._1)/2, bb1._4 - bb1._3)
        overlap(c_2)//no need for obj.overlap as we are in the class for this obj.
    }
    ...
    def rotate(angRad: Double): MyCanvas = new MyCanvas(listOfObjects.map(x => x.rotate(angRad)))
